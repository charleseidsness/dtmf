 /*
 *      DTMF Library, Based on Decoder from MultiMon
 *
...
#define TONES_BUFFER 32
#define FGETC_BUFFER TONES_BUFFER*4

#define BLOCKLENGTH 10/1000 /*10ms*/
#define BLOCKNUM 2
#define PHINC(x,y) ((x)*0x10000/y)
#define COS(x) costabf[(((x)>>6)&0x3ffu)]
#define SIN(x) COS((x)+0xc000)
#define MAX_SIDE_BAND	0.2
...
static char fgetc_buffer[FGETC_BUFFER];
...
static inline int process_block(int block_length)
{
	float tote;
	float totte[16];
	int i, j;
	...
	for (i = 0; i < 16; i++) {
		...
		for (j = 0; j < BLOCKNUM; j++)
			totte[i] += state.tenergy[j][i];
	}
	for (i = 0; i < 8; i++)
		totte[i] = (totte[i]*totte[i]) + (totte[i+8]*totte[i+8]);
	memmove(state.energy+1, state.energy, 
		sizeof(state.energy) - sizeof(state.energy[0]));
	state.energy[0] = 0;
	memmove(state.tenergy+1, state.tenergy, 
		sizeof(state.tenergy) - sizeof(state.tenergy[0]));
	memset(state.tenergy, 0, sizeof(state.tenergy[0]));
	tote *= (BLOCKNUM*block_length*0.5);  /* adjust for block lengths */
...
static char* dtmf_demod(unsigned int sample_rate, float *buffer, int length, char * tones)
{
	float s_in;
	int i;
	int block_length = (sample_rate*BLOCKLENGTH);
	unsigned int dtmf_phinc[8] = {
		PHINC(1209,sample_rate), PHINC(1336,sample_rate),
		PHINC(1477,sample_rate), PHINC(1633,sample_rate),
		PHINC(697,sample_rate), PHINC(770,sample_rate),
		PHINC(852,sample_rate), PHINC(941,sample_rate)
	};
	unsigned int hits = 0;
	for (; length > 0; length--, buffer++) {
		s_in = *buffer;
		state.energy[0] += (s_in*s_in);
		for (i = 0; i < 8; i++) {
			state.tenergy[0][i] += COS(state.ph[i]) * s_in;
			state.tenergy[0][i+8] += SIN(state.ph[i]) * s_in;
			state.ph[i] += dtmf_phinc[i];
		}
		if ((state.blkcount--) <= 0) {
			state.blkcount = block_length;
			i = process_block(block_length);
			if (i != state.lastch && i >= 0) {
				tones[hits++]=dtmf_transl[i];
			}
			state.lastch = i;
		}
	}
...
{
        short buffer[READ_SIZE];
	float fbuf[READ_SIZE<<1];
	unsigned int fbuf_cnt = 0;
	int i = 0;
	int j = 0;
	int pound = 0, eol = 0;
	char tones_buffer[TONES_BUFFER] = { '\0' };

	tones[0] = '\0';

	do {
#ifdef DTMF_ALSA
	snd_pcm_readi(fd, (unsigned char *)buffer, READ_SIZE);
	i = READ_SIZE*sizeof(short);
#else
	i = read(*fd, buffer, READ_SIZE*sizeof(short));
#endif
		for (fbuf_cnt=0; fbuf_cnt< (i/2 - 1) ; fbuf_cnt++)
			fbuf[fbuf_cnt] = buffer[fbuf_cnt]*(1.0/32768.0);
		dtmf_demod(SAMPLE_RATE, fbuf, fbuf_cnt, tones_buffer);
		i = 0;
		eol = 0;
		do {
			switch (tones_buffer[i]) {
			case ('\0'):
				eol = 1;
				break;
			case ('#'):
				...
				break;
			default:
				putchar(tones_buffer[i]);
				fflush(stdout);
				tones[j++] = tones_buffer[i++];
				tones[j] = '\0';
				break;
			}
		} while (i < TONES_BUFFER && !pound && !eol && j < tones_length);
	} while(j < tones_length && !pound);
	return tones;
}
...
char *dtmf_fgetl(char * tones, unsigned int tones_length, DTMF_FILE * fd)
{
	...
	for (fbuf_cnt=0; fbuf_cnt< (i/2 - 1) ; fbuf_cnt++)
		fbuf[fbuf_cnt] = buffer[fbuf_cnt]*(1.0/32768.0);
	dtmf_demod(SAMPLE_RATE, fbuf, fbuf_cnt, tones);
	return tones;
}
...
char dtmf_fgetc(DTMF_FILE * fd)
{
	...
	for (fbuf_cnt=0; fbuf_cnt< (i/2 - 1) ; fbuf_cnt++)
		fbuf[fbuf_cnt] = buffer[fbuf_cnt]*(1.0/32768.0);
	dtmf_demod(SAMPLE_RATE, fbuf, fbuf_cnt, tones_buffer);
	strcat(fgetc_buffer, tones_buffer);
	memmove(&fgetc_buffer[0], &fgetc_buffer[1], sizeof(fgetc_buffer)-1);
	return fgetc_buffer[0];
}
...
#ifdef DTMF_ALSA
int dtmf_fclose(DTMF_FILE *pcm_handle)
{
...
DTMF_FILE * dtmf_fopen(char * pcm_name, char * rw)
{
	...
	unsigned int dir= -1;
	snd_pcm_uframes_t periodsize = READ_SIZE;
	snd_pcm_uframes_t buffersize = READ_SIZE<<2;

	if (strcmp(rw,"r")) {
		printf("Only read has been implimented.\n");
		return NULL;
	}
	...
	if(snd_pcm_hw_params_set_rate(pcm_handle, hwparams, SAMPLE_RATE, 0) < 0) {
		dtmf_fclose(pcm_handle);
		return NULL;
	}
...
#else
DTMF_FILE * dtmf_fopen(char * device,  char * rw)
{
	int * fd;
	int sndparam;
